...
def parse_args():
    ...
    parser.add_argument(
        "-t", "--test_image", type=str, default=None, help="Test image location"
    )
    parser.add_argument(
        "-c",
        "--checkpoint_location",
    ...
    parser.add_argument(
        "-e", "--epoch", type=int, default=120, help="Epoches to run training"
    )
    ...
    parser.add_argument(
        "--d_lr", type=float, default=1e-4, help="learning rate for discriminator"
    )
    parser.add_argument(
        "-i",
        "--checkpoint_every",
    ...
    parser.add_argument(
        "--d_init", type=str, default=None, help="Init weights for discriminator"
    )
    parser.add_argument(
        "--g_init", type=str, default=None, help="Init weights for generator"
    )
...
if args.test_image is not None:
    ...
    cv2.imwrite(
        os.path.join(args.checkpoint_location, "test_init.jpg"),
...
for epoch in range(args.epoch):
    for y, uv in training_generator:
        ...
        valid = Variable(
            torch.Tensor(y.size(0), 1).fill_(1.0), requires_grad=False
        ).cuda(args.gpu)
        fake = Variable(
            torch.Tensor(y.size(0), 1).fill_(0.0), requires_grad=False
        ).cuda(args.gpu)
        ...
        if i % args.checkpoint_every == 0:
            print(
                "Epoch: %d: [D loss: %f] [G total loss: %f] [G GAN Loss: %f]"
                % (epoch, d_loss.item(), g_loss.item(), g_loss_gan.item())
            )

            torch.save(
                D.state_dict(),
                os.path.join(
                    args.checkpoint_location, "weights", "D" + str(epoch) + ".pth"
                ),
            )
            torch.save(
                G.state_dict(),
                os.path.join(
                    args.checkpoint_location, "weights", "G" + str(epoch) + ".pth"
                ),
            )
            if args.test_image is not None:
                test_res = G(test_var)
                ...
                cv2.imwrite(
                    os.path.join(
                        args.checkpoint_location, "test_epoch_" + str(epoch) + ".jpg"
                    ),
                    (test_rgb.clip(min=0, max=1) * 256)[:, :, [2, 1, 0]],
                )
